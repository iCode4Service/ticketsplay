...
/** Scenario: Show balances of all coins in account with TKTS.**/
// Create a new API wrapper instance and call to the balances and rates commands.
try {
    $cps_api = new TicketsplayAPI($private_key, $public_key, 'json');
...
if ($balances['error'] == 'ok' && $rates['error'] == 'ok') {
    ...
    $output = '<table><tbody><tr><td>Currency</td><td>Ticketscoin Balance</td><td>Floating Point Balance</td><td>USD value</td></tr>';
    ...
    if (!empty($positive_balances)) {
        $usd_to_TKTS = $rates['result']['USD']['rate_TKTS'];
        foreach ($positive_balances as $currency => $positive_balance) {
            $this_currency_to_TKTS = $rates['result'][$currency]['rate_TKTS'];
            $positive_balances[$currency]['usd_value'] = round($positive_balances[$currency]['balancef'] * ($this_currency_to_TKTS / $usd_to_TKTS), 2);
        }
    }
